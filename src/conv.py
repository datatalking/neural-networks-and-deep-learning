...
def ensemble(nets):
    ...
    for net in nets:
        ...
        net.test_mb_predictions = theano.function(
            ...
            net.layers[-1].y_out,
            givens={
                net.x: test_x[i * net.mini_batch_size : (i + 1) * net.mini_batch_size]
            },
        )
        net.test_predictions = list(
            np.concatenate([net.test_mb_predictions(i) for i in xrange(1000)])
        )
    ...
    error_locations = [
        j for j in xrange(10000) if plurality_test_predictions[j] != test_y_eval[j]
    ]
